...
- The default multiplex setup for the Duo and Duo 256 provides:
  - 2 Hardware PWMs: GP4, GP5
  - 2 Hardware I2Cs: GP0+GP1, GP11+GP10
  - 2 Hardware UARTs: GP2+GP3 (boot console), GP12+GP13
  - 1 hardware SPI: GP6+GP7+GP8,GP9
  - 12 GPIOs: GP14-GP22, GP25, GP26, GP27
...
- Use the `duo-pinmux` tool to customize these to suit your needs. See official documentation [here](https://milkv.io/docs/duo/application-development/pinmux).
  - **Note:** What the diagrams refer to as `SPI2` (for Duo and Duo 256M) becomes `/dev/spidev0` in Linux, so give `index: 0` (or no index at all) to use that hardware SPI. This is also likely the case for `SPI3` on the Duo S, but that is untested yet.
